...
const static char *dateformat0 = "%FT%T%z";
...
{
    struct tm tmd0 {};
    memset(&tmd0, 0, sizeof(struct tm));
    bool invFmt = strptime(v, dateformat0, &tmd0) == nullptr;
    if (invFmt) {
        invFmt = strptime(v, dateformat1, &tmd0) == nullptr;
        if (invFmt)
        ...
        if (tmd0.tm_gmtoff == 0) {
            struct tm tmd1{};
            memset(&tmd1, 0, sizeof(struct tm));
            strptime(v, dateformat1, &tmd1) == nullptr;
            if (tmd1.tm_gmtoff != 0)
                return mktime(&tmd1);
        }
        return mktime(&tmd0);
